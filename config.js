...
// ✅ Decks & Hands (Persist across reloads)
...
export async function loadAllCards() {
    ...
    try {
        ...
        const characterFiles = [
            "./data/beast-chars.json",
            "./data/bully-chars.json",
            "./data/celestial-chars.json",
            "./data/hero-chars.json",
            "./data/life-chars.json",
            "./data/mystical-chars.json",
            "./data/olympian-chars.json",
            "./data/plant-chars.json",
            "./data/underworld-chars.json",
            "./data/water-chars.json",
        ];

        // Load all card data asynchronously
        ...
        // Check if any deck failed to load
        ...
        // Prevent reloading decks if they already exist
        if (playerDeck.length === 0) {
            playerDeck.push(...shuffleDeck([...fullDeck]));
        }
        if (enemyDeck.length === 0) {
            enemyDeck.push(...shuffleDeck([...fullDeck]));
        }

        console.log("✅ Decks Loaded Successfully!");
        console.log("🎴 Player Deck:", playerDeck.length, "cards");
        console.log("🎴 Enemy Deck:", enemyDeck.length, "cards");
    } catch (error) {
